...
from vpython import scene, rate, vector
from vpython import *
G = 6.67 * 10 ** -11
...
class RevolvingSystem:
  #  """This class represents a central body with satellites orbiting around it"""
    def __init__(self, centralBody):
      ...
      ##  The constructor the Revolving System class.
       ...
       # """
        ...
        def toggle_follow(evt):
            self.follow_earth = not self.follow_earth
            print(f"Follow Earth: {'ON' if self.follow_earth else 'OFF'}")

        scene.bind("keydown", toggle_follow)
    ...
    def simulate(self, dt):
        ...
        dt *= 36 * 24  # time scaling
        self.follow_earth = True  # Start with following Earth
        ...
        # Initial camera view
        if body_to_follow:
            scene.camera.pos = body_to_follow.pos + vector(0, 1e10, 3e10)
            scene.camera.axis = body_to_follow.pos - scene.camera.pos
            scene.center = body_to_follow.pos
        ...
        def toggle_follow(evt):
            ...
            if key == 'f':
                self.follow_earth = not self.follow_earth
                print(f"[TOGGLE] Follow Earth: {'ON' if self.follow_earth else 'OFF'}")
        ...
        while True:
            ...
            for obj in self.canvas.objects:
                if obj.name != self.centralBody.name:
                    self.updateForce(obj)
                    self.updateMomentum(obj, dt)

            self.updatePositions(dt)

            if self.follow_earth and body_to_follow:
                scene.center = body_to_follow.pos
    ...
    def updatePositions(self, dt):
        ...
        for x in range(len(self.canvas.objects)):
            ...
            # self.canvas.camera.pos = vector(0, 0, 3e11)  # Camera position
            # self.canvas.camera.axis = vector(0, 0, -1)   # Direction it’s pointing
            # self.canvas.up = vector(0, 1, 0)             # "Up" direction